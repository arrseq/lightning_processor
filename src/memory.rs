...
#[derive(Debug, PartialEq)]
pub struct Aligned<'a, Memory: Seek> {
    pub memory: &'a mut Memory,
    unaligned_access_error: bool
}

impl<'a, Memory: Seek> Aligned<'a, Memory> {
    ...
    fn handle_alignment(&mut self, buf: &[u8]) -> Result<(), io::Error> {
...
impl<'a, Memory: Seek + Read> Read for Aligned<'a, Memory> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        self.handle_alignment(buf)?;
        self.unaligned_access_error = false;
        self.memory.read(buf)
    }
}

impl<'a, Memory: Seek + Write> Write for Aligned<'a, Memory> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        self.handle_alignment(buf)?;
...
impl<'a, Memory: Seek> Seek for Aligned<'a, Memory> {
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        self.memory.seek(pos)
    }
}